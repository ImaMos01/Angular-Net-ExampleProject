...
export class AuthorAutocompleteComponent {
  ...
  endDragItem(event:CdkDragDrop<any[]>){
    const prevIndex = this.selectedAuthors.findIndex(it => it === event.item.data);
    moveItemInArray(this.selectedAuthors, prevIndex,event.currentIndex);
    this.table.renderRows();
  }
